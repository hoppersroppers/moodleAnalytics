...
class cohort_analysisWeekly():
    ...
    def analyzeWeek(self):
        ...
        """
        self.input_df['Purchase_month'] = self.input_df[self.conversion_date_column].apply(lambda x: x.to_period('W'))

        if(1):
            print('Creating Cohorts')
            # create cohort based on first month, if cohort is not known
            first_month_df = self.input_df.groupby(self.customer_id_column)[self.conversion_date_column].apply(lambda x: min(x).to_period('W'))
    ...
    def plot_retention(self,cmap='RdYlBu'):
        ...
        if setFlag == "count":
            sns.heatmap(self.retention_table,annot=True,cmap=cmap)
        else:
